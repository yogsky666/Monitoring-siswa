            ...
            <div class="flex-none w-full max-w-full px-3">
                <div
                    ...
                    <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                        <h6 class="text-blue-700 text-xl font-bold">Tambah Data Bimbingan</h6>
                    </div>

                    <!-- Form -->
                    <form action="{{ route('bimbingan.store') }}" method="POST" class="flex-auto p-6 space-y-6">
                        @csrf
                        ...
                        <br>
                        <div>
                            <label for="id_perbaikan" class="block mb-2 text-sm font-bold text-gray-700">KODE
                                PERBAIKAN</label>
                            <input type="text" name="id_perbaikan" id="id_perbaikan"
                                class="focus:shadow-primary-outline text-sm focus:border-blue-500 block w-xl rounded-lg border border-gray-300 bg-white p-3 leading-5 text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:ring"
                                placeholder="Kode Perbaikan" required value="{{ old('id_perbaikan') }}">
                            @error('id_perbaikan')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
